function rebuild_selection_cache(_panel, _entity) {
    ...
    var _transponder_off = variable_struct_exists(_info, "transponder") && !_info.transponder;

    if (_transponder_off) {
        var _line = "SHIP UNKNOWN";
        _panel.lines = [_line];

        var _text_width = string_width(_line);
        _panel.w = _text_width + BOX_PAD_X * 2;
        _panel.h = _panel.line_height + BOX_PAD_TOP + BOX_PAD_BOTTOM;
    ...
    _panel.w = _text_width + BOX_PAD_X * 2;
    _panel.h = (_panel.line_height * _total_count) + BOX_PAD_TOP + BOX_PAD_BOTTOM;
    _panel.scale_x = _panel.w / _panel.box_w;
    _panel.scale_y = _panel.h / _panel.box_h;
}